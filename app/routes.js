...
module.exports = function(app) {
  ...
  //***reommendations from MOVIE search bar****
    ...
    app.get('/api/movieSearch', function(request, response) {
      ...
      var query2 = "SELECT m.title as title1, mg.genre, m.rating AS rating1 FROM cis550.movie m JOIN cis550.MOVIE_GENRES mg ON m.id = mg.movie_id JOIN (SELECT genre FROM cis550.MOVIE_GENRES g WHERE movie_id = (SELECT id FROM cis550.movie WHERE title=" + request.body.name +") LIMIT 1) g ON g.genre = mg.genre WHERE rating >=8 ORDER BY RAND() LIMIT 10;"
      connection.query(query2, request.params.user, function(err, res) {
      if (err)
        response.send({err: err});
      var movieResult = [];
